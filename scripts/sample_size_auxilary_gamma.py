...
from bandit import ShiftedSymmetricSimulationDistribution, ShiftedExponentialSimulationDistribution
...
def base_evaluate_tree_symmetric(iterate, epsilon, epsilon_m, gamma, n, n_m, d, M, K, kappa, if_weighted):
    ...
    mab = ShiftedSymmetricSimulationDistribution(K, dim, gamma_list, kappa)
    X_source_list, arms_source_list, y_source_list, _ = mab.generate_source(n_source_list)
...
def run_base_auxilary_gamma():
    ...
    #         for gamma in [ 0, 0.2, 0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6, 1.8, 2.0]:
    #             for n in [  40000]:
                            ...
                            for M in [2]:
                                for K in [2]:
                                    for kappa in [1]:
                                        for if_weighted in [True]:
                                            print("epsilon: {}, epsilon_m: {}, gamma: {}, n: {}, n_m: {}, d: {}, M: {}, K: {}, kappa: {}, if_weighted: {}".format(epsilon, epsilon_m, gamma, n, n_m, d, M, K, kappa, if_weighted))
                                            Parallel(n_jobs = num_jobs, verbose = 30)(delayed(base_evaluate_tree_symmetric)(iterate, epsilon, epsilon_m, gamma, n, n_m, d, M, K, kappa, if_weighted) for iterate in range(num_repetitions))
